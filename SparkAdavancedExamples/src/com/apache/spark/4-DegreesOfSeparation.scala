...
object DegreesOfSeparation {
  ...
  def main(args: Array[String]) {
    ...
    val context = new SparkContext("local[*]", "DegreesOfSeparation") 
    
    // Our accumulator, used to signal when we find the target 
    // character in our BFS traversal.
    hitCounter = Some(context.longAccumulator("Hit Counter"))
    
    var iterationRdd = createStartingRdd(context)
  ...
  def createStartingRdd(context:SparkContext): RDD[BFSNode] = {
    val inputFile = context.textFile("../data/marvel-graph.txt")
